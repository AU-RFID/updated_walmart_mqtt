...
class RFIDReadingProcessor:
    def __init__(self, broker_address, broker_port,
        ...
        if (rfid_receiver and rfid_analyzer) is True:
            raise Exception("Sorry, receiver and analyzer are mutual exclusive.")
        if (rfid_receiver or rfid_analyzer) is False:
            raise Exception("Sorry, receiver and analyzer are mutual exclusive.")
        ...
        if len(cafile + certfile + keyfile) > 0:
        ...
        if rfid_receiver is True:
            self.client.on_message = self.on_message_receive_rfid
        if rfid_analyzer is True:
            self.client.on_message = self.on_message_analyze_rfid

        # Connect to the broker
        self.client.connect(self.broker_address, self.broker_port)

        # Queues for message handling, and start standby threads for handling sensor data and requests
    ...
    def on_message_receive_rfid(self, client, userdata, msg):
        if msg.topic == config.TOPICLIST['tag_event']:
            self.rfid_reading_queue.put(msg.payload)

    def on_message_analyze_rfid(self, client, userdata, msg):
        if msg.topic == config.TOPICLIST['RFID_req']:
    ...
    def retrieve_data_from_DB_with_Time(self, start_time, end_time):
        ...
        print(f'query takes {time.time()-query_start}s, {len(result)} reads are collected.')
        if not result.empty:
            # for debugging the NONE entry at the bottom of te list
            tmp_res = result[-50:]
            print('result:\n', tmp_res)
            st = int(tmp_res.iloc[0]['timestamp:eventNum'].split(':')[0])
            et = int(tmp_res.iloc[-1]['timestamp:eventNum'].split(':')[0])
            print('the reading rate:', len(tmp_res)/((et-st)/1000))
        return result
    ...
    def prepare_rfid_data(self, request_msg):
        ...
        rfid_start_time = msg_dict['payload']['SessionStartTime']
        rfid_end_time = msg_dict['payload']['SessionEndTime']
    ...
    def rfid_request_reporter(self, data_to_publish, msg_template, start_time, end_time):
        ...
        :return: N/A
        """
        msg_template['payload']['SessionStartTime'] = start_time
        msg_template['payload']['SessionEndTime'] = end_time
